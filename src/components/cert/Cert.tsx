...
function Cert({ name, title, org, date }: any) {
	const { ref, fontSize } = useFitText({ maxFontSize: 250, minFontSize: 1 });
	return (
	...
	return (
		<div className='cert-desc'>
			<p ref={ref} style={{ fontSize }}>
				for actively participating and completing the community event "{title}"
				conducted on {date}. This event was initiated by {org} and hosted by
				AltruWiz Inc.
			</p>
		</div>
	);
